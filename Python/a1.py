...
def light_pixels(image, lightness, channel):
     """Returns a mask for each channel that identifies the pixels whose intensity is 
     above the given threshold. The mask of channel i is an array that has values 0 or 1.
     Assuming that the image is stored as a numpy array with three channels for the colours
     'red', 'green', and 'blue'.
...
def histogram(image, buckets, channel):
    """
    Returns a histogram of the channel, where the image is represented as a 
    ...
    array([2, 0, 0, 1, 0, 3])
    """
    # Map the channel name to the corresponding index in the third dimension
    channel_map = {'red': 0, 'green': 1, 'blue': 2}
    channel_index = channel_map[channel]
    
    # Extract the specified channel's data from the image
    channel_data = image[:, :, channel_index]
    
    # Initialize the histogram array with zeros
    histogram_array = np.zeros(buckets, dtype=int)
    
    # Calculate the bucket size
    bucket_size = 256 // buckets
    
    # Iterate over all pixels in the channel data
    for pixel_value in channel_data.flatten():
        # Determine the appropriate bucket for the pixel value
        bucket_index = min(pixel_value // bucket_size, buckets - 1)
        # Increment the corresponding bucket count
        histogram_array[bucket_index] += 1
...
def build_deep_nn(rows, columns, channels, layer_options):
     ...
     """Returns a Keras neural model that has the following layers:
     ...
     """
     model = Sequential()
     model.add(Input(shape=(rows, columns, channels)))
     model.add(Flatten())
